<template>
  <div class="vampire-response">
    ...
    <div class="vampire-response__editor-container">
      <div ref="editorRef" class="vampire-response__editor vampire-parchment-bg" />
    </div>
...
function saveDraft(content: string) {
  try {
    localStorage.setItem(`vampire_draft_${props.promptId}`, content)
  } catch (e) {
...
function loadDraft(): string | null {
  try {
    return localStorage.getItem(`vampire_draft_${props.promptId}`)
  } catch {
    return null
  }
}
...
async function handleSubmit() {
  ...
  try {
    ...
    // 清除草稿
    localStorage.removeItem(`vampire_draft_${props.promptId}`)
...
.vampire-response__editor {
  min-height: 200px;
}

.vampire-response__editor-content {
  min-height: 200px;
  outline: none;
}
